class DayValidator < ActiveModel::Validator
	...
	def no_other_record_exist(record)
		if record.type_of_day == 1
			Day.by_type_within_date(1, record.date_from, record.date_to).empty?
		else
			Day.by_type_within_date(2, record.date_from, record.date_to).where("user_id = ?", record.user_id).empty?
		end
	end
end