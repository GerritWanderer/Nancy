class DaySequence < ActiveRecord::Base
  ...
  scope :find_type_by_user_and_year, lambda {|sequence_type, user, year| where(:type_of_sequence => sequence_type, :user_id => user).where("date_from BETWEEN '?-01-01' AND '?-12-31'", year, year) }
  scope :find_holidays_by_year, lambda {|year| where(:type_of_sequence => 1).where("date_from BETWEEN '?-01-01' AND '?-12-31'", year, year).order('date_to ASC') }

  def self.count_holidays_by_year(year)
    holiday_records = DaySequence.where(:type_of_sequence => 1).where("date_from BETWEEN '?-01-01' AND '?-12-31'", year, year)
