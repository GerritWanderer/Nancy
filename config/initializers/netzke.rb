Netzke::Railz::Engine.class_eval do
  ...
  config.after_initialize do
    ...
    File.open(Rails.root.join('tmp', 'netzke', 'ext.js'), 'w') {|f| f.write(Netzke::Core::DynamicAssets.ext_js) }
    File.open(Rails.root.join('tmp', 'netzke', 'ext.css'), 'w') {|f| f.write(Netzke::Core::DynamicAssets.ext_css) }
    File.open(Rails.root.join('tmp', 'netzke', 'touch.js'), 'w') {|f| f.write(Netzke::Core::DynamicAssets.touch_js) }
    File.open(Rails.root.join('tmp', 'netzke', 'touch.css'), 'w') {|f| f.write(Netzke::Core::DynamicAssets.touch_css) }
  end
end